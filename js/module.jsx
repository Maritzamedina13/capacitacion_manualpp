...
function ModuleFlow({ module, quiz, alreadyDone, onComplete, onExit }) {
  ...
  if (phase === 'intro') {
    return (
      <div className="stage" style={{ ...rootStyle, background: `linear-gradient(160deg,${c.c1},${c.c2})`, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, opacity: .09, backgroundImage: 'radial-gradient(circle at 80% 15%, #fff 0, transparent 40%), radial-gradient(circle at 10% 90%, #fff 0, transparent 35%)' }} />
        <header className="no-print" style={{ position: 'relative', padding: 'clamp(18px,2.4vw,28px) clamp(20px,5vw,54px)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <ITMMark light compact />
          <button onClick={onExit} className="btn" style={{ background: 'rgba(255,255,255,.15)', color: '#fff', padding: '10px 18px', fontSize: 14 }}><Icon name="arrowL" size={16} /> Mapa</button>
        </header>
        ...
        <main style={{ position: 'relative', flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(16px,4vw,40px)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'auto minmax(0,1fr)', gap: 'clamp(24px,5vw,60px)', alignItems: 'center', maxWidth: 1000 }} className="intro-grid">
            ...
            <div style={{ color: '#fff' }}>
              <div className="chip" style={{ background: 'rgba(255,255,255,.18)', color: '#fff', marginBottom: 14, whiteSpace: 'nowrap' }}>MÓDULO {module.id} · {module.cap}</div>
              <h1 style={{ fontSize: 'clamp(30px,4.4vw,50px)', fontWeight: 900, lineHeight: 1.04 }}>{module.title}</h1>
              <p style={{ fontSize: 'clamp(15px,1.7vw,18px)', color: 'rgba(255,255,255,.85)', marginTop: 12, lineHeight: 1.55, maxWidth: 540 }}>{module.subtitle}</p>

              <div style={{ background: 'rgba(255,255,255,.12)', borderRadius: 16, padding: '18px 20px', marginTop: 22, maxWidth: 560, backdropFilter: 'blur(4px)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontWeight: 800, fontSize: 13, letterSpacing: '.05em', textTransform: 'uppercase', opacity: .9, whiteSpace: 'nowrap' }}><Icon name="target" size={16} /> Objetivo de aprendizaje</div>
                <p style={{ marginTop: 10, fontSize: 14.5, lineHeight: 1.5, color: 'rgba(255,255,255,.95)' }}>{module.objective}</p>
              </div>

              <div style={{ marginTop: 18 }}>
                <div style={{ fontWeight: 800, fontSize: 13, letterSpacing: '.05em', textTransform: 'uppercase', opacity: .85, marginBottom: 10 }}>Al terminar serás capaz de…</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {module.learn.map((l, i) => (
                    <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', fontSize: 14, color: 'rgba(255,255,255,.92)' }}>
                      <span style={{ width: 22, height: 22, borderRadius: '50%', background: 'rgba(255,255,255,.22)', display: 'grid', placeItems: 'center', flexShrink: 0, marginTop: 1 }}><Icon name="check" size={13} stroke={3} /></span>
                      {l}
                    </div>
                  ))}
                </div>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 28, flexWrap: 'wrap', alignItems: 'center' }}>
                <button className="btn" style={{ background: '#fff', color: c.c1, fontSize: 16 }} onClick={() => { setSlide(0); setPhase('content'); }}>
                  <Icon name="play" size={16} /> {alreadyDone ? 'Repasar módulo' : 'Empezar a aprender'}
                </button>
                <span style={{ fontSize: 13, color: 'rgba(255,255,255,.75)', fontWeight: 600 }}>{module.content.length} temas · {total} retos · meta {PASS_PCT}%</span>
              </div>
            </div>
          </div>
        </main>
  ...
  /* ---------------- RESULTADO ---------------- */
  return (
    <div className="stage" style={{ ...rootStyle, background: passed ? `linear-gradient(160deg,${c.c1},${c.c2})` : 'var(--paper-2)', position: 'relative', overflow: 'hidden' }}>
      {passed && <Confetti run />}
      <main style={{ position: 'relative', flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 'clamp(20px,4vw,40px)' }}>
        <Reveal variant="pop" className="card" style={{ maxWidth: 520, width: '100%', padding: 'clamp(28px,4vw,44px)', textAlign: 'center' }}>
          {passed ? (
            <>
              <Reveal variant="spin" delay={150} style={{ display: 'inline-block' }}><Badge module={module} size={150} state="earned" /></Reveal>
              <div className="chip" style={{ background: c.soft, color: c.c1, margin: '18px auto 0' }}><Icon name="sparkle" size={14} /> ¡Insignia desbloqueada!</div>
              <h1 style={{ fontSize: 'clamp(26px,3.4vw,34px)', fontWeight: 900, color: 'var(--ink)', marginTop: 14 }}>{module.badge.tag}</h1>
              <p style={{ color: 'var(--ink-soft)', fontSize: 15.5, marginTop: 8, lineHeight: 1.5 }}>Superaste el módulo <strong>{module.title}</strong> con {scorePct}% de aciertos.</p>
              <div style={{ display: 'flex', justifyContent: 'center', gap: 18, margin: '22px 0' }}>
                ...
                <div><div style={{ fontSize: 26, fontWeight: 900, color: 'var(--gold-deep)' }}>+{correctCount * 100 + (module.id === 6 ? 200 : 150)}</div><div style={{ fontSize: 12, color: 'var(--ink-soft)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.05em' }}>XP ganados</div></div>
              </div>
              <button className="btn btn-primary" style={{ width: '100%', justifyContent: 'center' }} onClick={() => onComplete(scorePct, correctCount * 100 + (module.id === 6 ? 200 : 150))}>
            ...
            <>
              ...
              <p style={{ color: 'var(--ink-soft)', fontSize: 15, marginTop: 8, lineHeight: 1.5 }}>Necesitas al menos <strong>{PASS_PCT}%</strong> para ganar la insignia. Repasa el contenido e inténtalo de nuevo.</p>
