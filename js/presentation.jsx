      ...
      <div className="exec-slide exec-cover" style={{
        ...
        <div style={{
          ...
          <img src="recursos/logo-itm-2.png" alt="ITM"
            style={{ height:'clamp(52px,6.5vw,85px)', marginBottom:'8%', objectFit:'contain', alignSelf:'flex-start' }} />

          <h1 style={{ color:'#fff', fontSize:'clamp(29px,4vw,55px)', fontWeight:900, lineHeight:1.08, marginBottom:'4%' }}>
          ...
          <p style={{ color:'rgba(255,255,255,0.72)', fontSize:'clamp(16px,1.6vw,20px)', lineHeight:1.7, maxWidth:360 }}>
          ...
          <div style={{ marginTop:'auto', paddingTop:'8%', color:'rgba(255,255,255,0.35)', fontSize:'clamp(13px,1.2vw,15px)', fontWeight:600 }}>
        ...
        <div className="exec-cover-right" style={{
          ...
          <div style={{ fontWeight:800, fontSize:'clamp(16px,1.5vw,19px)', color:'#102D69', letterSpacing:'.1em', textTransform:'uppercase' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
            {[
              ...
              <div key={l} style={{ display:'flex', alignItems:'center', gap:'4%', padding:'3.5% 0', borderBottom:'1px solid #e8ecf5' }}>
                <div style={{ width:'clamp(43px,5vw,58px)', height:'clamp(43px,5vw,58px)', borderRadius:10, background:'linear-gradient(135deg,#102D69,#00A0B7)', display:'grid', placeItems:'center', color:'#fff', flexShrink:0 }}>
                  <Icon name={icon} size={22} />
                </div>
                <div>
                  <div style={{ fontWeight:900, fontSize:'clamp(24px,2.9vw,38px)', color:'#102D69', lineHeight:1 }}>{n}</div>
                  <div style={{ fontWeight:600, fontSize:'clamp(14px,1.3vw,17px)', color:'#8a96b0', marginTop:2 }}>{l}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display:'flex', gap:'clamp(4px,0.8vw,10px)', flexWrap:'wrap', paddingTop:'2%' }}>
            {modules.slice(0, 5).map(m => (
              <Badge key={m.id} module={m} size={Math.min(67, window.innerWidth * 0.052)} state="earned" showNumber={false} />
            ))}
          </div>
        </div>
      </div>
    ...
    return (
      <div className="exec-slide" style={{ ...vis(idx), background:'#fff' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'2% 4% 1.5%', borderBottom:`1px solid ${c.c1}18`, flexShrink:0 }}>
          <img src="recursos/logo-itm-1.png" alt="ITM" style={{ height:'clamp(35px,4.4vw,58px)', objectFit:'contain' }} />
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <div style={{ width:37, height:37, borderRadius:'50%', background:`linear-gradient(135deg,${c.c1},${c.c2})`, display:'grid', placeItems:'center', color:'#fff', fontWeight:900, fontSize:17 }}>{m.id}</div>
            <div style={{ textAlign:'right' }}>
              <div style={{ fontWeight:800, fontSize:'clamp(15px,1.5vw,19px)', color:c.c1, letterSpacing:'.06em', textTransform:'uppercase' }}>{m.cap}</div>
              <div style={{ fontWeight:600, fontSize:'clamp(14px,1.25vw,16px)', color:'#8a96b0', letterSpacing:'.04em', textTransform:'uppercase' }}>Prácticas Profesionales ITM</div>
            </div>
          </div>
        </div>
        ...
        <div className="exec-chapter-body" style={{ flex:1, display:'grid', gridTemplateColumns:'26% 1fr', gap:'2% 3%', padding:'2% 4%', minHeight:0 }}>
          <div className="exec-badge-col" style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:'5%' }}>
            <Badge module={m} size={Math.min(144, window.innerWidth * 0.115)} state="earned" showNumber />
            <div style={{ textAlign:'center' }}>
              <div style={{ fontWeight:800, fontSize:'clamp(14px,1.3vw,16px)', color:c.c1, letterSpacing:'.08em', textTransform:'uppercase' }}>Insignia</div>
              <div style={{ fontWeight:900, fontSize:'clamp(16px,1.56vw,19px)', color:'#16203a', marginTop:2 }}>{m.badge.tag}</div>
            </div>
          </div>

          <div style={{ display:'flex', flexDirection:'column', justifyContent:'center', gap:'3%', minWidth:0 }}>
            <div>
              <h2 style={{ fontSize:'clamp(21px,2.6vw,32px)', fontWeight:900, color:'#16203a', lineHeight:1.1, marginBottom:'1%' }}>{m.title}</h2>
              <p style={{ fontSize:'clamp(15px,1.4vw,18px)', color:'#56608a', fontStyle:'italic', lineHeight:1.4 }}>{m.subtitle}</p>
            </div>

            <div style={{ background:`${c.c1}0c`, border:`1px solid ${c.c1}22`, borderLeft:`3px solid ${c.c1}`, borderRadius:'0 8px 8px 0', padding:'2% 3%' }}>
              <div style={{ fontWeight:800, fontSize:'clamp(14px,1.3vw,16px)', color:c.c1, letterSpacing:'.08em', textTransform:'uppercase', marginBottom:'1.5%' }}>Objetivo</div>
              <p style={{ fontSize:'clamp(15px,1.4vw,18px)', color:'#16203a', lineHeight:1.5 }}>{m.objective}</p>
            </div>

            <div>
              <div style={{ fontWeight:800, fontSize:'clamp(14px,1.3vw,16px)', color:'#4a5675', letterSpacing:'.08em', textTransform:'uppercase', marginBottom:'1.5%' }}>Competencias clave</div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'1.5% 2%' }}>
                {m.learn.slice(0,4).map((p, i) => (
                  <div key={i} style={{ display:'flex', gap:'4%', alignItems:'flex-start' }}>
                    <span style={{ width:22, height:22, borderRadius:'50%', background:`linear-gradient(135deg,${c.c1},${c.c2})`, display:'grid', placeItems:'center', flexShrink:0, marginTop:2 }}>
                      <Icon name="check" size={15} stroke={3} style={{ color:'#fff' }} />
                    </span>
                    <span style={{ fontSize:'clamp(14px,1.3vw,16px)', color:'#16203a', lineHeight:1.4 }}>{p}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'1% 4%', borderTop:`1px solid ${c.c1}15`, flexShrink:0 }}>
          <span style={{ fontSize:'clamp(13px,1.2vw,15px)', color:'#8a96b0', fontWeight:600 }}>Manual de Prácticas Profesionales ITM · MDE 001 V5</span>
          <span style={{ fontSize:'clamp(13px,1.2vw,15px)', color:c.c1, fontWeight:700 }}>{idx} / {TOTAL - 1}</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="exec-slide" style={{ ...vis(TOTAL - 1), background:'linear-gradient(145deg,#050d20,#0a1e4a 50%,#102D69)' }}>
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'4% 6%', position:'relative', textAlign:'center', gap:'3%' }}>
          <div style={{ fontWeight:800, fontSize:'clamp(15px,1.5vw,19px)', color:'#56ACDE', letterSpacing:'.14em', textTransform:'uppercase' }}>Ruta de Aprendizaje</div>
          <h2 style={{ color:'#fff', fontSize:'clamp(27px,3.75vw,50px)', fontWeight:900, lineHeight:1.1, maxWidth:600 }}>
            ¡Ahora estás listo para comenzar tu ruta!
          </h2>
          <p style={{ color:'rgba(255,255,255,0.7)', fontSize:'clamp(16px,1.6vw,20px)', maxWidth:520, lineHeight:1.65 }}>
          ...
          <div style={{ display:'flex', gap:'clamp(5px,1vw,12px)', flexWrap:'wrap', justifyContent:'center' }}>
            {modules.map(m => (
              <Badge key={m.id} module={m} size={Math.min(72, window.innerWidth * 0.058)} state="earned" showNumber={false} />
            ))}
          </div>
          <div style={{ display:'flex', gap:'clamp(18px,2.8vw,44px)', flexWrap:'wrap', justifyContent:'center' }}>
            {[['10 000','XP máximo'],['9','Insignias'],['1','Constancia']].map(([n,l]) => (
              <div key={l} style={{ textAlign:'center' }}>
                <div style={{ color:'#f4c84a', fontWeight:900, fontSize:'clamp(24px,3.1vw,37px)', lineHeight:1 }}>{n}</div>
                <div style={{ color:'rgba(255,255,255,0.5)', fontWeight:600, fontSize:'clamp(14px,1.3vw,16px)', textTransform:'uppercase', letterSpacing:'.06em', marginTop:3 }}>{l}</div>
              </div>
            ))}
          </div>
        </div>
        <div style={{ padding:'1.5% 5%', display:'flex', justifyContent:'space-between', borderTop:'1px solid rgba(255,255,255,0.1)', color:'rgba(255,255,255,0.35)', fontSize:'clamp(13px,1.2vw,15px)', fontWeight:600 }}>
